...
const STORAGE_KEYS = {
    ...
    AUTOPLAY_PREFERENCE: "player_autoplay_preference", // 💡 自动播放偏好设置
};
...
function toggleAutoplayPreference() {
    // 切换当前自动播放偏好
    const newPreference = !getAutoplayPreference();
    
    // 保存到 localStorage
    if (typeof localStorage !== 'undefined') {
        localStorage.setItem(STORAGE_KEYS.AUTOPLAY_PREFERENCE, String(newPreference));
    }
    
    // 显示提示信息
    showErrorMessage(newPreference ? "已启用自动播放" : "已禁用自动播放");
}

// 💡 获取自动播放偏好设置
function getAutoplayPreference(): boolean {
    if (typeof localStorage !== 'undefined') {
        const saved = localStorage.getItem(STORAGE_KEYS.AUTOPLAY_PREFERENCE);
        if (saved !== null) {
            return saved === "true";
        }
    }
    // 如果没有保存过偏好，使用配置文件的默认值
    return musicPlayerConfig.autoplay ?? false;
}
...
onMount(() => {
    ...
    if (typeof localStorage !== 'undefined') {
        // 读取自动播放偏好
        const savedAutoplayPref = localStorage.getItem(STORAGE_KEYS.AUTOPLAY_PREFERENCE);
        if (savedAutoplayPref !== null) {
            isAutoplayEnabled = savedAutoplayPref === "true";
        } else {
            // 首次访问，使用配置文件默认值
            isAutoplayEnabled = musicPlayerConfig.autoplay ?? false;
        }
        ...
        <div class="flex items-center gap-4 mb-4">
            ...
            <div class="flex items-center gap-1">
                <button class="btn-plain w-8 h-8 rounded-lg flex items-center justify-center"
                        class:text-[var(--primary)]={getAutoplayPreference()}
                        onclick={toggleAutoplayPreference}
                        title={getAutoplayPreference() ? "禁用自动播放" : "启用自动播放"}>
                    <Icon icon="material-symbols:play-circle" class="text-lg" />
                </button>
                <button class="btn-plain w-8 h-8 rounded-lg flex items-center justify-center"
                        onclick={toggleMode}
