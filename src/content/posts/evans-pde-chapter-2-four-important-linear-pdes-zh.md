...
$$
\dot v(s)+c(z(s),s)v(s)=0.
$$

于是解会带上指数因子。这一点在后续一阶 PDE 和 Hamilton-Jacobi 方程中会反复出现。
...
在没有这类增长限制时，热方程可能出现非物理的零初值非零解。这反映热方程反向时间方向的病态性。
...
这个公式说明，三维非齐次波方程在 $(x,t)$ 的响应由过去光锥上的源项决定。
...
若初始数据在球 $B(x_0,t_0)$ 上为零，则解在整个光锥 $K(x_0,t_0)$ 中为零。
...
| 4 | 最大值原理的直接证明 | 对 $u+\varepsilon |x|^2$ 使用严格次调和扰动 |
